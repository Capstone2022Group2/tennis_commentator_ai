...
def main():
    # Example showing predictions being made on 'test_match.mp4' using a model saved as 'audio_model'
    model = tf.keras.models.load_model('../trained_models/audio_model')
    audio_file = AudioFileClip('../trained_models/test_vid/test_match.mp4')
    reader = audio_file.coreader().reader
    reader.seek(0)
    audio = reader.read_chunk(SAMPLE_SIZE)
    count = 0
